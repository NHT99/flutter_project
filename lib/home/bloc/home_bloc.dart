...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  HomeBloc() : super(HomeState(result: [], input: 0)) {
    ...
    on<DeleteNumberEvent>(_deleteNumber);
  }
  ...
  void _deleteNumber(DeleteNumberEvent event, Emitter<HomeState> emit) {
    final listResult = state.result;
    listResult?.removeAt(event.id);
    emit(state.copyWith(result: listResult));
  }
}
